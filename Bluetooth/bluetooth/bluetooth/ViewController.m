...
#define TRANSFER_SERVICE_UUID           @"3CC3"//@"FEE0"
...
@interface ViewController ()<CBCentralManagerDelegate,CBPeripheralDelegate>
{
    ...
    NSMutableArray *discoverPeripherals; //用于保存被发现设备
...
- (void)viewDidLoad {
    ...
    _atView = [[ATView alloc] init];
    _atView.center = self.view.center;
    _atView.bounds = CGRectMake(0, 0, 10, 10);
    [self.view addSubview:_atView];
    
    manager = [[CBCentralManager alloc] initWithDelegate:self queue:dispatch_get_main_queue() options:@{CBCentralManagerOptionShowPowerAlertKey:@YES}];
    discoverPeripherals = [[NSMutableArray alloc] initWithCapacity:0];
    [manager scanForPeripheralsWithServices:nil options:nil];
    
//    [[[ATCenterManager alloc] init] startScanPeripherals];
}
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    if (characteristic.value) {
        ...
        NSLog(@"-----%@---",tmpString);
//        NSLog(@"------>>>>value:%@---",[[NSString alloc] initWithData:characteristic.value encoding:NSUTF16StringEncoding]);
    }
}
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error{
    ...
    NSLog(@"didUpdateValueForDescriptor:%@  value:%@",[NSString stringWithFormat:@"%@",descriptor.UUID],descriptor.value);
}
